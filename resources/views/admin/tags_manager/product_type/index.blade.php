...
<script type="text/javascript">	
	$(document).ready(function() {
        $('#bootstrap-data-table').DataTable({
            ...
            ajax: '{{url('admin/producttype_list')}}',
            columns: [
                {data: 'name', name : 'name'},
    ...
    $(document).on('submit', '#producttype', function (event) {
        event.preventDefault();
        $('.error').empty();
        $('.addLoading').LoadingOverlay("show");
        axios.post("{{route('producttype.store')}}", $(this).serialize()).then(function (response) {
            ...
            $.each(error.response.data.errors, function (key, value) {
                $("#producttype").find("." + key).next("span.error").css({"color" : "red","font-size" : "85%"}).text(value)
            });
        });
    });
    $(document).on('click', "[id^='edit_']", function(){
        var id = $(this).prop('name');
        var url = window.app.url+'/admin/producttype/'+id+'/edit';
        axios.get(url)
    ...
    $(document).on('submit', '#producttype_edit', function (event) {
        event.preventDefault();
        $('.editloading').LoadingOverlay("show");
        $('.error').empty();
        var id = $('[name=id]').val();
        var url = window.app.url+'/admin/producttype/'+id;
        axios.put(url, $(this).serialize()).then(function (response) {
            ...
            $('.editloading').LoadingOverlay("hide"); 
            $.each(error.response.data.errors, function (key, value) {
                $("#disc_edit").find("." + key).next("span.error").css({"color" : "red","font-size" : "85%"}).text(value)
            });
        });
    });
</script>
