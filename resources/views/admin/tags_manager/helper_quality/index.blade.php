...
@push('breadcrumbs')
<li><a href="">Home</a></li>
<li class="active"> helper Quality</li>
@endpush
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h4 class="modal-title">Add Helper Quality </h4>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <!-- Modal body -->
            <form id="helperquality">
...
<script type="text/javascript">	
	$(document).ready(function() {
        $('#bootstrap-data-table').DataTable({
            ...
            ajax: '{{url('admin/helperquality_list')}}',
            columns: [
                {data: 'name', name : 'name'},
    ...
    $(document).on('submit', '#helperquality', function (event) {
        event.preventDefault();
        $('.error').empty();
        $('.addLoading').LoadingOverlay("show");
        axios.post("{{route('helperquality.store')}}", $(this).serialize()).then(function (response) {
            ...
            $.each(error.response.data.errors, function (key, value) {
                $("#helperquality").find("." + key).next("span.error").css({"color" : "red","font-size" : "85%"}).text(value)
            });
        });
    });
    $(document).on('click', "[id^='edit_']", function(){
        var id = $(this).prop('name');
        var url = window.app.url+'/admin/helperquality/'+id+'/edit';
        axios.get(url)
    ...
    $(document).on('submit', '#helperquality_edit', function (event) {
        event.preventDefault();
        $('.editloading').LoadingOverlay("show");
        $('.error').empty();
        var id = $('[name=id]').val();
        var url = window.app.url+'/admin/helperquality/'+id;
        axios.put(url, $(this).serialize()).then(function (response) {
            ...
            $('.editloading').LoadingOverlay("hide"); 
            $.each(error.response.data.errors, function (key, value) {
                $("#helperquality_edit").find("." + key).next("span.error").css({"color" : "red","font-size" : "85%"}).text(value)
            });
        });
    });
</script>
