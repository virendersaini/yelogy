...
Allergy Fields Manager

@endpush

@push('breadcrumbs')
<li><a href="">Home</a></li>
<li class="active"> Allergy Fields Manager</li>
@endpush
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h4 class="modal-title">Add Allergy Fields</h4>
...
<script type="text/javascript">	
    ...
    $(document).on('submit', '#allergy', function (event) {
        event.preventDefault();
        $('.error').empty();
        $('.addLoading').LoadingOverlay("show");
        axios.post("{{route('allergies.store')}}", $(this).serialize()).then(function (response) {
            ...
            $.each(error.response.data.errors, function (key, value) {
                $("#allergy").find("." + key).next("span.error").css({"color" : "red","font-size" : "85%"}).text(value)
            });
        });
    });
    $(document).on('click', "[id^='edit_']", function(){
        var id = $(this).prop('name');
        var url = window.app.url+'/admin/allergies/'+id+'/edit';
        axios.get(url)
    ...
    $(document).on('submit', '#allergy_edit', function (event) {
        event.preventDefault();
        $('.editloading').LoadingOverlay("show");
        $('.error').empty();
        var id = $('[name=id]').val();
        var url = window.app.url+'/admin/allergies/'+id;
        axios.put(url, $(this).serialize()).then(function (response) {
            ...
            $('.editloading').LoadingOverlay("hide"); 
            $.each(error.response.data.errors, function (key, value) {
                $("#allergy_edit").find("." + key).next("span.error").css({"color" : "red","font-size" : "85%"}).text(value)
            });
        });
    });
</script>
