...
<script type="text/javascript">	
    ...
    $(document).on('submit', '#disc', function (event) {
        event.preventDefault();
        $('.error').empty();
        $('.addLoading').LoadingOverlay("show");
        axios.post("{{route('discs.store')}}", $(this).serialize()).then(function (response) {
            ...
            $.each(error.response.data.errors, function (key, value) {
                $("#disc").find("." + key).next("span.error").css({"color" : "red","font-size" : "85%"}).text(value)
            });
        });
    });
    $(document).on('click', "[id^='edit_']", function(){
        var id = $(this).prop('name');
        var url = window.app.url+'/admin/discs/'+id+'/edit';
        axios.get(url)
    ...
    $(document).on('submit', '#disc_edit', function (event) {
        event.preventDefault();
        $('.editloading').LoadingOverlay("show");
        $('.error').empty();
        var id = $('[name=id]').val();
        var url = window.app.url+'/admin/discs/'+id;
